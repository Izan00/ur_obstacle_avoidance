...
class motionGeneration():
    ...
    def __init__(self):
        ...
        self.weights_file_path = os.getcwd()+"/src/roy_dmp/data/weights"
        self.rosbag_file_path = os.getcwd()+"/src/roy_dmp/data/rosbag_recordings"
    ...
    def loadMotionFromJointStates(self, bagname, joints,_dt=000.8,_K=100,_D=2.0 * np.sqrt(100),_num_bases=None, weight_name=None):
        ...
        self.info_bag = yaml.safe_load(subprocess.Popen(['rosbag', 'info', '--yaml', file],stdout=subprocess.PIPE).communicate()[0])
    ...
    def loadMotionYAML(self, yamlname):
        ...
        y_file = join(self.weights_file_path, yamlname)

        try:
            with open(y_file, 'r', encoding='utf-8') as stream:
                try:
                    # Converts yaml document to python object
                    motion_dict=yaml.load(stream, Loader=yaml.UnsafeLoader)
