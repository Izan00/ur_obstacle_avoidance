...
if __name__ == '__main__':
    ...
    coupling_term = CouplingTermObstacleAvoidance3D(obstacle, gamma=1000.0, beta=5.0 / math.pi)
    ...
    with open('dmp.pkl', 'wb') as file: 
        # A new file will be created 
        pickle.dump([waypoints], file) 

    #waypoints.pop(0)    
    print(len(waypoints))
    #waypoints2 = [[waypoints[i] for i in range(101,len(waypoints),50)][-10]]
    #waypoints2 += [waypoints[i] for i in range(101,len(waypoints),50)][-8:]
    #waypoints2 = [[waypoints[i] for i in range(101,len(waypoints),50)][-9]]
    waypoints2 = [waypoints[i] for i in range(0,len(waypoints),500)]
    print(waypoints2)
    print(len(waypoints2))
    print('Computed')
